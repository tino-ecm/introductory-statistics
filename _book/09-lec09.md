...
*  1変量$x$の分散は\textcolor{red}{一直線上にあるデータのちらばり}の指標
\begin{align*} 
\text{Var}(x)
%s_x^2
...
*  2変量$x,y$の場合，ヨコ軸方向の偏差$x_i-\bar{x}$とタテ軸方向の偏差$y_i-\bar{y}$が定義できるので\textcolor{red}{平面的な散らばり}が表現できる
%\begin{align*}
...
1.  値の下限は$-1$，上限は$1$~~$\Rightarrow$~~\textcolor{red}{$-1 \le \text{Corr}(x,y) \le 1$}
2.  2変量間の\textcolor{red}{線形的な関係の強さ}を測る指標
3.  データの単位に影響を受けない指標
